...
import { MapPin, Route, Play, Navigation2, RefreshCw, Eye, EyeOff } from 'lucide-react';

const ControlsPanel = ({ mode, setMode, resetGraph, runPrimsAlgorithm, isAnimating, nodes, edges, mstEdges, showOnlyMst, setShowOnlyMst }) => {
  return (
    <div className="card toolbar" style={{ flexShrink: 0 }}>
      ...
      {mstEdges?.length > 0 && (
        <button
          className={`btn ${showOnlyMst ? 'btn-primary' : 'btn-secondary'}`}
          onClick={() => setShowOnlyMst(!showOnlyMst)}
          disabled={isAnimating}
          style={{ width: '100%', marginBottom: '1rem' }}
        >
          {showOnlyMst ? <EyeOff size={16} /> : <Eye size={16} />}
          {showOnlyMst ? 'Show All Roads' : 'View Minimum Spanning Tree'}
        </button>
      )}
